    ...
    <Dropdown
      :items="
        props.buttons.map((b) => ({
          text: b.label,
          onClick: () => b.onClick?.(),
          danger: b.type === 'danger',
        }))
      "
    >
      <button
        class="inline-flex items-center justify-center w-9 h-9 rounded-md border border-gray-400 dark:border-gray-500 text-dark hover:bg-gray-100 dark:text-white dark:hover:bg-gray-700 cursor-pointer"
      >
        <Io5Menu size="18" />
      </button>
    </Dropdown>
