...
const props = withDefaults(
  defineProps<{
    ...
    error?: boolean
    success?: boolean
    onChange?: (value: string | { startDate: string | null; endDate: string | null }) => void
    range?: boolean
  }>(),
  {
...
const emit = defineEmits<{
  ...
  (e: 'change', value: string | { startDate: string | null; endDate: string | null }): void
}>()
...
const inputClasses = computed(() => {
  ...
  if (props.solid) {
    ...
    if (props.darkDarker) {
      classList.push('dark:bg-[#0F131C]', 'focus:dark:bg-[#06070A]')
    } else if (props.darkLighter) {
      classList.push('dark:bg-dark-2', 'focus:dark:bg-dark-3')
    } else {
      classList.push('dark:bg-dark', 'focus:dark:bg-dark-darker')
    }
  } else {
