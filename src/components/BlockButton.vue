...
const baseClasses =
  'w-full rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-dark-2 shadow-soft transition hover:shadow-md inline-flex flex-col items-center justify-center text-center select-none cursor-pointer hover:border-gray-300 dark:hover:border-gray-500'
...
const processedIconVNodes = computed(() => {
  ...
  return nodes.map((vnode, idx) =>
    cloneVNode(vnode, {
      class: [vnode.props?.class || '', sizeClasses],
      key: vnode.key ?? idx,
    })
  )
})
