...
const activeColor = computed(() => props.items[currentIndex.value]?.color ?? null)
...
<template>
  <div>
    ...
    <div
      class="md:hidden border-b-3"
      :class="activeColor ? '' : 'border-gray-100 dark:border-dark-2'"
    ...
    >
      ...
      <div v-show="isCollapsedOpen" class="pb-2">
        <button
          v-for="(tab, index) in props.items"
          :key="index"
          type="button"
          class="block w-full text-left px-3 py-2 rounded text-sm cursor-pointer"
          :class="
            currentIndex === index
              ? 'bg-gray-2 dark:bg-white/10 text-black dark:text-white'
