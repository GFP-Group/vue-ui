...
import { EpWarning } from 'vue-icons-plus/ep'
...
const panelClasses = computed(() =>
  props.visible ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 translate-y-4 scale-95'
)
...
<template>
  <div
    class="fixed top-0 left-0 flex items-center justify-center w-full h-full min-h-screen px-4 py-5 bg-dark/70 dark:bg-[#8094BD]/40 z-50 transition-opacity duration-200"
    :class="overlayClasses"
    @click.self="props.closeOnOutside ? setVisible(false) : undefined"
  >
    <div
      ref="modalContainer"
      class="w-full max-w-[450px] rounded-lg bg-white dark:bg-dark py-8 px-6 text-center md:py-[42px] md:px-[60px] mt-[-15vh] transform transition-all duration-300"
      :class="[panelClasses, props.className]"
      role="dialog"
      aria-modal="true"
    >
      <div
      ...
      <h3 v-if="props.title" class="pb-3 text-xl font-medium text-dark dark:text-white sm:text-2xl">
        {{ props.title }}
      </h3>
      <p
        v-if="props.message"
