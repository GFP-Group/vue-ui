...
const props = withDefaults(
  defineProps<{
    ...
    ariaLabel?: string
    // When true, button has no bg/border by default, but shows them on hover
    naked?: boolean
  }>(),
  {
    size: 20,
...
const baseClasses = computed(() => {
  if (props.naked) {
    return 'inline-flex items-center justify-center rounded-full aspect-square border border-transparent hover:border-gray-200 dark:hover:border-gray-700 transition hover:bg-gray-100 dark:hover:bg-gray-700/20 text-dark dark:text-gray-200 p-0 leading-none shrink-0 overflow-hidden cursor-pointer'
  }
  return 'inline-flex items-center justify-center rounded-full aspect-square border border-gray-200 dark:border-gray-700 shadow-soft transition hover:shadow-md text-dark dark:text-gray-200 p-0 leading-none shrink-0 overflow-hidden'
})
...
const buttonClasses = computed(() => {
  ...
  return `${baseClasses.value} ${props.bgClass ?? ''}`
})
