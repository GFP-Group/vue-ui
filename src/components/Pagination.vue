...
const showJumpToLast = computed(() => {
  const max = Math.max(1, props.maxVisible || 5)
  const lastVisible = visiblePages.value[visiblePages.value.length - 1]
  return totalPages.value > max && lastVisible < totalPages.value
})
      ...
      <li v-for="page in visiblePages" :key="page">
        <button
          ...
          :class="[
            ...
            page === internal
              ? 'bg-gray-2 text-black hover:brightness-110 dark:bg-gray-700 dark:text-white dark:border-r-transparent'
