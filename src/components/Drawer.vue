...
<template>
  ...
  <div
    ref="panelRef"
    class="fixed top-0 z-50 h-screen w-[85vw] md:w-full md:max-w-[400px] bg-white dark:bg-dark-2 py-6 px-5 sm:px-9 shadow-xl transition-transform duration-300 will-change-transform flex flex-col"
    :class="[
  ...
  >
    ...
    <!-- Fixed Header -->
    <div v-if="props.title" class="mb-4 flex justify-between flex-shrink-0">
      <Text h3>{{ props.title }}</Text>
    </div>

    <!-- Scrollable Content -->
    <div class="flex-1 overflow-y-auto space-y-[30px]">
      <slot />
    </div>

    <!-- Fixed Footer -->
    <div v-if="$slots.footer" class="pt-5 flex-shrink-0">
      <slot name="footer" />
    </div>
  </div>
</template>
